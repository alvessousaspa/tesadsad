...
gasusage = int(configfile.gasusage)
    ...
    class Uniswap:
        def __init__(
            ...
            if self.version == 1:
                ...
                self.router = self._load_contract(
                    abi_name="uniswap-v2/router02ava", address=self.router_address,
                )
            else:
        ...
        def _eth_to_token_swap_input(
            ...
            if self.version == 1:
                ...
                return self._build_and_send_tx(gwei, my_address,my_pk,
                    self.router.functions.swapExactAVAXForTokens(
                        amount_out_min,
        ...
        def _token_to_eth_swap_input(
            ...
            if self.version == 1:
                ...
                return self._build_and_send_tx(gwei, my_address,my_pk,
                    self.router.functions.swapExactTokensForAVAX(
                        qty,
                        amount_out_min,
        ...
        def _eth_to_token_swap_output(
            ...
            if self.version == 1:
                ...
                eth_qty = self.get_eth_token_output_price(output_token, qty)
                return self._build_and_send_tx(gwei, my_address,my_pk,
                    self.router.functions.swapAVAXForExactTokens(
                        qty,
                        [self.get_weth_address(), output_token],
                        recipient,
                        self._deadline(),
        ...
        def _token_to_eth_swap_output(
            ...
            if self.version == 1:
                ...
                return self._build_and_send_tx(gwei, my_address,my_pk,
                    self.router.functions.swapTokensForExactAVAX(
                        qty,
                        max_tokens,
                        [input_token, self.get_weth_address()],
                        self.address,
        ...
        # ------ Approval Utils ------------------------------------------------------------
        def _get_tx_params2(self, value: Wei = Wei(0), gas: Wei = Wei(gasusage)) -> TxParams:
        ...
        def approve(self, token: AddressLike, max_approval: Optional[int] = None) -> None:
            ...
            self.w3.eth.waitForTransactionReceipt(tx, timeout=3000)
        ...
        def _get_tx_params(self,gwei,my_address, value: Wei = Wei(0), gas: Wei = Wei(gasusage)) -> TxParams:
            """Get generic transaction parameters."""
            return {
                "from": my_address,
                "value": value,
